...
def create_efiction_converter(ini_file_name: str):
    ...
    test_config = ArchiveConfig(
        test_logger, ini_file_name, "efiction/tests/test_data"
    ).config
    test_sql = SqlDb(test_config, test_logger)
    test_config["Processing"]["working_dir"] = get_full_path(
...
def load_fixtures(test_config, test_sql):
    ...
    test_sql.load_sql_file_into_db(
        get_full_path(test_config["Processing"]["original_tidied_file"])
    )
