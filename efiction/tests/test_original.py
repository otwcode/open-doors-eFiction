...
test_config = ArchiveConfig(
    MagicMock(), "efiction_no_defs", "efiction/tests/test_data"
).config
...
class TestOriginal(TestCase):
    ...
    def test_load_original_file(self, mock_add_create_database):
        ...
        test_sql.load_sql_file_into_db.arg_should_contain(
            "efictiontestnodefs_efiction_original_edited.sql"
        )
    ...
    def test_add_table_definitions(self):
        ...
        self.assertTrue(
            result[4].strip().startswith("CREATE TABLE `fanfiction_stories`"),
