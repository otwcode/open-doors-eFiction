...
def get_data():
    with open(
        get_full_path("efiction/tests/test_data/efiction_chapters.json"),
        "r",
        encoding="utf-8",
    ) as file:
...
def mock_insert(mock_sql):
    with patch("efiction.chapters.BigInsert", autospec=True) as mock_Insert:
        insert = mock_Insert(
            "efictiontest_working_open_doors",
            "chapters",
...
def test_chapters_with_text_main_method(mock_method, mock_sql, mock_insert):
    ...
    mock_sql.read_table_with_total.return_value = (
        old_data_with_text,
        0,
        len(old_data_with_text),
    )
    ...
    assert efiction_chapters.load_chapters(
        "test_path"
    ), "load_chapters doesn't return true"
...
def test_chapters_with_text_processing(mock_sql, mock_insert):
    ...
    assert len(new_data_full) == len(
        working_chapters
    ), "Returned chapters from method don't match the number of expected chapters"

    mock_sql.read_table_to_dict.assert_called_with(
        "efictiontest_test_step_original_efiction_edited", "chapters"
    )
    mock_sql.read_table_with_total.assert_called_with(
        "efictiontest_test_step_original_efiction_edited", "chapters"
    )
    mock_sql.execute_and_fetchall.assert_called_with(
        "efictiontest_working_open_doors", "SELECT * FROM chapters;"
    )

    assert mock_insert.addRow.call_count == len(
        new_data_full
    ), "addRow calls don't match the number of expected chapters"
...
def test_chapters_no_text_processing(monkeypatch, mock_sql, mock_insert):
    ...
    efiction_chapters = EFictionChapters(test_config, test_logger, mock_sql)
    working_chapters = efiction_chapters.load_chapter_text_into_db(
        efiction_chapters.list_chapter_files()
    )
    assert len(new_data_missing) == len(
        working_chapters
    ), "Returned chapters from method don't match the number of expected chapters"
