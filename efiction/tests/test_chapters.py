...
def get_data():
    ...
    for i, chap in enumerate(old_data_with_text):
        ...
        new_data.append(chap_dict)

    return old_data_with_text, old_data_no_text, new_data


old_data_with_text, old_data_no_text, new_data = get_data()
...
def mock_sql():
    with patch('opendoors.mysql.SqlDb', autospec=True) as mock_Sql:
        ...
        sql.execute_and_fetchall.return_value = new_data
        yield sql
...
def test_chapters_with_text_main_method(mock_method, mock_sql, mock_insert):
    ...
    assert efiction_chapters.load_chapters("test_path")
...
def test_chapters_with_text_processing(mock_sql, mock_insert):
    ...
    assert len(new_data) == len(working_chapters)
    ...
    assert mock_insert.addRow.call_count == len(new_data)
...
def test_chapters_no_text_processing(mock_sql, mock_insert):
    mock_sql.read_table_to_dict.return_value = old_data_no_text
    mock_sql.read_table_with_total.return_value = (old_data_no_text, 0, len(old_data_no_text))
