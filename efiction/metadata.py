...
class EFictionMetadata:
    ...
    def __init__(
        self, config: ConfigParser, logger: Logger, sql: SqlDb, step_path: str
    ):
        self.config = config
        self.sql = sql
        self.logger = logger
        self.working_original = (
            self.config["Processing"]["simplified_original_db"]
            if self.config.has_option("Processing", "simplified_original_db")
            else None
        )
    ...
    def create_open_doors_db(self, step_path):
        ...
        statements = add_create_database(
            self.config["Processing"]["open_doors_working_db"], od_table_defs
        )
        od_tables = write_statements_to_file(
            self.config["Processing"]["open_doors_working_db_file"], statements
        )
    ...
    def _convert_authors(self, old_authors=None):
        ...
        insert_op = BigInsert(
            self.working_open_doors, "authors", ["id", "name", "email"], self.sql
        )
    ...
    def _convert_characters(self, old_characters):
        ...
        for old_character in old_characters:
            parent = [
                ct["original_tag"]
                for ct in self.tag_tables["categories"]
                if ct["original_tagid"] == old_character["catid"]
            ]
            new_tag = {
            ...
            insert_op.addRow(
                new_tag["id"], new_tag["name"], "character", new_tag["parent"]
            )
    ...
    def _convert_story_tags(self, old_story):
        old_tags = {
            ...
            "categories": [
                t.strip() for t in key_find("catid", old_story, "").split(",")
            ],
            "warnings": [t.strip() for t in key_find("wid", old_story, "").split(",")],
            "classes": [
                t.strip() for t in key_find("classes", old_story, "").split(",")
            ],
            "genres": [t.strip() for t in key_find("gid", old_story, "").split(",")],
            "characters": [
                t.strip() for t in key_find("charid", old_story, "").split(",")
            ],
        }
        ...
        for tag_table_name in self.tag_tables.keys():
            new_tags[tag_table_name] = self._convert_story_tag_table(
                tag_table_name, old_tags
            )
    ...
    def convert_all_tags(self):
        ...
        old_characters = self.sql.read_table_to_dict(
            self.working_original, "characters"
        )
        self.tag_tables["characters"] = self._convert_characters(old_characters)

        self.tag_table_is_nonstandard = (
            self.tag_converter.check_for_nonstandard_tag_tables()
        )
    ...
    def convert_original_to_open_doors(self, step_path: str):
        ...
        """
        self.logger.info(
            "Converting metadata tables from eFiction to Open Doors structure..."
        )
        ...
        if not self.config.has_option("Archive", "language_code"):
            language = input(
                "Two-letter Language code of the stories in this archive (default: en - press enter):\n>> "
            )
        ...
        database_dump = get_prefixed_path(
            "03", step_path, f"{self.working_open_doors}_without_chapters.sql"
        )
