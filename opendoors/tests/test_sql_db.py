...
class TestSqlDb(TestCase):
    ...
    test_config: ConfigParser = ArchiveConfig(
        test_logger, "test", "opendoors/tests/test_data"
    ).config
    ...
    def tearDown(self) -> None:
        ...
        filtered = [
            f
            for f in glob.glob("opendoors/tests/test_output/*")
    ...
    def test_dump_database(self):
        self.test_sql.load_sql_file_into_db(
            get_full_path("opendoors/tests/test_data/test.sql")
        )
        self.test_sql.dump_database(
            "od_test_sql", get_full_path("opendoors/tests/test_output/test_output.sql")
        )
